...
/**
 * Testes unitários do HabitoService.
 * Dependências isoladas com Mockito — sem banco de dados.
 */
...
class HabitoServiceTest {
    ...
    void setUp() {
        ...
        habitoSalvo.setPontosGerados(300);
    ...
    void deveRegistrarHabitoComSucesso() {
        ...
        assertThat(response.getPontosGerados()).isEqualTo(300); // 30 * 10
        assertThat(response.getColaboradorId()).isEqualTo("COL-001");
        verify(habitoRepository).save(any(RegistroHabito.class));
    }

    @Test
    @DisplayName("Deve lançar exceção para colaborador inexistente")
    ...
    @Test
    @DisplayName("Deve retornar lista de todos os hábitos")
    void deveListarTodosOsHabitos() {
        ...
        assertThat(lista.get(0).getTipoAtividade()).isEqualTo(TipoAtividade.CORRIDA);
    }
    ...
    void deveLancarExcecaoParaIdInexistente() {
        when(habitoRepository.findById(99L)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> habitoService.buscarPorId(99L))
                .isInstanceOf(RecursoNaoEncontradoException.class);
    }

    @Test
    @DisplayName("Deve deletar hábito existente com sucesso")
